...
class Solution:
    def findTheCity(self, n: int, edges: List[List[int]], distanceThreshold: int) -> int:
        ...
        min_reachable = n+1
        for i in range(n):
            reachable = [j for j in range(n) if dists[i][j] <= distanceThreshold and j != i]
...
@pytest.mark.parametrize('args', [
    ...
    ((6, [[2,3,7],[2,5,8],[0,2,8],[4,5,5],[1,5,10],[3,4,3],[0,5,9],[1,2,1]], 3269, 5)),
])
def test(args):
    assert args[-1] == Solution().findTheCity(*args[:-1])
