...
class Solution:
    def maxProduct(self, root: Optional[TreeNode]) -> int:
        ...
        def sum_tree(node):
            if not node:
                return 0
            cur = (node.val + sum_tree(node.left) + sum_tree(node.right)) # % MOD
            nonlocal ret, s
    ...
    def maxProduct(self, root: Optional[TreeNode]) -> int:
        ...
        def summation(node, total_sum=None):
            if not node:
                return 0
            left = summation(node.left, total_sum)
            right = summation(node.right, total_sum)
            if total_sum is not None:
                nonlocal ret
                ret = max(ret, (total_sum - left) * left, (total_sum - right) * right)
            return node.val + left + right

        summation(root, summation(root))
        return ret%(10**9+7)
    ...
    def maxProduct(self, root: Optional[TreeNode]) -> int:
        ...
        def dfs(node, callback=lambda x: x):
            if not node:
                return 0
            s = node.val + dfs(node.left) + dfs(node.right)
            if total:
                nonlocal ret
                ret = max(ret, s*(total-s))
            return s
