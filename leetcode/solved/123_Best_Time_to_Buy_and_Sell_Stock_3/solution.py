...
class Solution:
    ...
    def _maxProfit(self, prices):
        self.prices = prices
        self.build_peaks_and_bottoms()
        # print(self.peaks)
        # print(len(self.peaks))
        # print(self.bottoms)
        # print(len(self.bottoms))
        return self.profit(-1, 2)
    ...
    def profit(self, i, n):
        ...
        for b in self.bottoms:
            ...
            for p in self.peaks:
                if p <= b: continue
                gap = self.prices[p] - self.prices[b]
                # print('gap: ', b, p, gap)
                sub = self.profit(p+1, n-1)
                # print('sub: ', b, p, sub)
    ...
    def build_peaks_and_bottoms(self):
        prices = self.prices
        self.peaks = []
        self.bottoms = []
        for i in range(len(prices)-1, -1, -1):
            if self.is_peak(i):
                self.peaks.append(i)
            if self.is_bottom(i):
                self.bottoms.append(i)
        self.peaks.reverse()
        self.bottoms.reverse()
