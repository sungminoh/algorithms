...
class UndergroundSystem:
    ...
    def checkOut(self, id: int, stationName: str, t: int) -> None:
        start_station_name, start_time = self.passenger.pop(id)
        key = (start_station_name, stationName)
        takes = t - start_time
        self.times.setdefault(key, [0, 0.])
        cnt, avg = self.times[key]
        self.times[key] = (cnt+1, (avg*cnt/(cnt+1)) + (takes/(cnt+1)))

    def getAverageTime(self, startStation: str, endStation: str) -> float:
        return self.times[(startStation, endStation)][1]
...
def test(commands, arguments, expecteds):
    ...
    assert pytest.approx(expecteds[1:], 0.0001) == actuals
