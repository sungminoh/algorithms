...
class TimeMap:
    ...
    def set(self, key: str, value: str, timestamp: int) -> None:
        ts = self.d.setdefault(key, [])
        i = bisect.bisect(ts, timestamp, key=lambda x: x[0])
        v = (timestamp, value)
        if i == len(ts):
            ts.append(v)
        elif ts[i][0] == timestamp:
            ts[i] = v
        else:
            # Unless the timestamps are strictly increasing
            # this needs improvement
            ts.insert(i, v)

    def get(self, key: str, timestamp: int) -> str:
        ts = self.d.get(key, [])
        i = bisect.bisect_right(ts, timestamp, key=lambda x: x[0])
        if i == 0:
            return ''
        return ts[i-1][1]
