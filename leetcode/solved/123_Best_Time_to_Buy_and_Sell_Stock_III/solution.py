...
class Solution:
    def maxProfit(self, prices):
        ...
        def profit(i, n):
            ...
            for b in bottoms:
                ...
                for p in peaks:
                    if p <= b: continue
                    gap = prices[p] - prices[b]
                    # print('gap: ', b, p, gap)
                    sub = profit(p+1, n-1)
                    # print('sub: ', b, p, sub)
                    # print(gap, sub, gap+sub)
                    m = max(m, gap + sub)
            # print('ret: ', i, n, m)
            return m

        def build_peaks_and_bottoms():
            nonlocal bottoms, peaks
        ...
        def is_peak(i):
            return (i == 0 or prices[i-1] <= prices[i]) \
                and (i == len(prices)-1 or prices[i] > prices[i+1])

        def is_bottom(i):
            return (i == 0 or prices[i-1] >= prices[i]) \
                and (i == len(prices)-1 or prices[i] < prices[i+1])

        build_peaks_and_bottoms()
        return profit(-1, 2)
    ...
    def maxProfit(self, prices):
        ...
        for i in range(len(prices)-1, -1, -1):
            p = prices[i]
            ma = max(ma, p)
            right[i] = max(right[(i+1)%len(prices)], ma - p)
        print(left, right)
    ...
    def maxProfit(self, prices: List[int]) -> int:
        ...
        @lru_cache(None)
        def dp(i, r, position):
            if i >= len(prices) or not r:
                return 0
            if position == 1:
                return max(prices[i] + dp(i+1, r-1, 0), dp(i+1, r, 1))
            else:
                return max(-prices[i] + dp(i+1, r, 1), dp(i+1, r, 0))
    ...
    def maxProfit(self, prices: List[int]) -> int:
        """10/21/2021 14:52"""
        left = [0]*len(prices)
        min_from_left = float('inf')
        for i in range(len(prices)):
            left[i] = max(left[i-1] if i>0 else 0, prices[i] - min_from_left)
            min_from_left = min(min_from_left, prices[i])

        right = [0]*len(prices)
        max_from_right = -float('inf')
        for i in range(len(prices)-1, -1, -1):
            right[i] = max(right[i+1] if i<len(prices)-1 else 0, max_from_right - prices[i])
            max_from_right = max(max_from_right, prices[i])

        return max(l + r for l, r in zip(left, right))

    def maxProfit(self, prices):
        """Cleanest from others"""
        sell1, buy1, sell2, buy2 = 0, -float('inf'), 0, -float('inf')
        for i in range(len(prices)):
