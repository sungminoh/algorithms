...
class Solution:
    def calculateMinimumHP(self, dungeon):
        """12/10/2018 01:21"""
        memo = [[None] * len(dungeon[0]) for _ in dungeon]
        ...
        def foo(i, j):
            ...
            sub = []
            if i < len(dungeon) - 1:
                sub.append(foo(i + 1, j))
            if j < len(dungeon[0]) - 1:
                sub.append(foo(i, j + 1))
            here = min(sub) - dungeon[i][j]
    ...
    def calculateMinimumHP(self, dungeon: List[List[int]]) -> int:
        ...
        def dp(i, j, v):
            if i == 0 and j == 0:
                return max(1, v - dungeon[i][j])
            ret = []
            if i > 0:
                ret.append(dp(i-1, j, max(1, v-dungeon[i][j])))
            if j > 0:
                ret.append(dp(i, j-1, max(1, v-dungeon[i][j])))
            return min(ret)

        return dp(len(dungeon)-1, len(dungeon[0])-1, 1)
    ...
    def calculateMinimumHP(self, dungeon: List[List[int]]) -> int:
        ...
        for i in range(m-1, -1, -1):
            for j in range(n-1, -1, -1):
                ...
                if j < n-1:
                    buf.append(dp[j+1])
                dp[j] = max(1, min(buf) - dungeon[i][j])

        return dp[0]


@pytest.mark.parametrize('dungeon, expected', [
...
def test(dungeon, expected):
    assert expected == Solution().calculateMinimumHP(dungeon)
